...
import {players,knownCompositionCount} from '../data/players.js?v=titan-bounds-4';import {t} from './i18n.js';
...
function render(){const hero=players.reduce((s,p)=>s+(p.heroPower||0),0), titan=players.reduce((s,p)=>s+(p.titanPower||0),0),activity=players.reduce((s,p)=>s+p.activityTotal,0);
 ...
 const root=document.querySelector('[data-known]');if(root) root.innerHTML=players.filter(p=>p.heroTeam?.length).sort((a,b)=>(b.heroPower+b.titanPower)-(a.heroPower+a.titanPower)).slice(0,6).map(p=>`<a class="mini-player" href="players.html"><b>${p.nickname}</b>${heroTeamMarkup(p.heroTeam)}${p.titanTeam?.length?titanTeamMarkup(p.titanTeam):''}<div class="meta">${t('totalPower')}: ${fmt((p.heroPower||0)+(p.titanPower||0))}</div></a>`).join('');
}
